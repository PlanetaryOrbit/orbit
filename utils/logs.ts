        ...
        try {
          ...
          const embed: EmbedObject = {
            ...
            description: userInfo
              ? `**${userInfo.username}** performed **${
                  userAction.toLowerCase() == 'audit log'
                    ? 'a workspace change'
...
export async function queryAudit(
  workspaceGroupId: number,
  opts: { userId?: number; action?: string; search?: string; skip?: number; take?: number } = {},
) {
  ...
  try {
    ...
    if (opts.search) {
      clauses.push(
        `(LOWER("action") LIKE LOWER($${idx}) OR LOWER(COALESCE("entity", '')) LIKE LOWER($${idx}) OR LOWER(COALESCE(CAST("details" AS TEXT), '')) LIKE LOWER($${idx}))`,
      );
    ...
    const total =
      Array.isArray(countRes) && countRes[0]
        ? Number(countRes[0].cnt || countRes[0].count || 0)
        : 0;
