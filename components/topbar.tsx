...
import {
  IconLogout, IconChevronDown, IconSun, IconMoon, IconSettings,
  IconX, IconDeviceLaptop, IconDeviceMobile, IconDeviceDesktop,
  IconDevices, IconChevronLeft, IconTrash
...
const Topbar: NextPage = () => {
  ...
  async function logout() {
    ...
    setLogin({ userId: 1, username: '', displayname: '', canMakeWorkspace: false, thumbnail: '', workspaces: [], isOwner: false, isFirstLogin: false });
    router.push('/login');
  }
  ...
  async function unlink() {
    ...
    } catch { toast.error("Failed to unlink", { id }); }
  }
          ...
          <div className="flex justify-between items-center h-16">
            ...
            >
              <div className="relative">
                <img src={login?.thumbnail} className="h-8 w-8 rounded-full bg-zinc-200 dark:bg-zinc-600 object-cover" alt="" />
              </div>
              <span className="text-sm font-medium text-zinc-700 dark:text-zinc-200 hidden sm:block">{login?.displayname}</span>
                ...
                {panel === 'main' && (
                  <div>
                    <div className="p-4 border-b border-zinc-100 dark:border-zinc-700/60">
                      <div className="flex items-center gap-3">
                        <div className="relative shrink-0">
                          <img src={login?.thumbnail} className="h-12 w-12 rounded-xl object-cover bg-zinc-200/10" alt="" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="font-semibold text-zinc-900 dark:text-white text-sm truncate">{login?.displayname}</p>
                          <p className="text-xs text-zinc-500 dark:text-zinc-400 truncate">@{login?.username}</p>
                        </div>
                        {login?.isOwner && (
                          <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300 shrink-0">
                            Owner
                          </span>
                        )}
                      </div>
                    ...
                    <div className="p-2 space-y-0.5">
                      ...
                      <button onClick={() => { openPanel('sessions'); fetchSessions(); }}
                        className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-700/60 transition-colors text-left">
                ...
                {panel === 'sessions' && (
                  <div>
                    <div className="flex items-center gap-2 p-4 border-b border-zinc-100 dark:border-zinc-700/60">
                      ...
                      <p className="font-semibold text-sm text-zinc-900 dark:text-white flex-1">Active sessions</p>
                        ...
                        <div className="p-2 space-y-1">
                          {sessions.map(s => (
                            <div key={s.id}
                              ...
                              <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${s.isCurrent ? 'bg-primary/10 text-primary' : 'bg-zinc-100 dark:bg-zinc-700 text-zinc-500 dark:text-zinc-400'}`}>
                ...
                {panel === 'settings' && (
                  <div>
                    <div className="flex items-center gap-2 p-4 border-b border-zinc-100 dark:border-zinc-700/60">
                      ...
                      <p className="font-semibold text-sm text-zinc-900 dark:text-white">Account settings</p>
                    </div>

                    <div className="p-4 space-y-3">
                      <div className="flex items-center gap-4">
                        <img src={login?.thumbnail} alt="" className="h-16 w-16 rounded-2xl object-cover bg-zinc-200 dark:bg-zinc-600 shrink-0" />
                        <div>
                          <p className="font-semibold text-zinc-900 dark:text-white">{login?.displayname}</p>
                        ...
                        login.discordUser ? (
                          <div className="flex items-center gap-3 p-3 rounded-xl border border-zinc-200 dark:border-zinc-600/60 bg-zinc-50 dark:bg-zinc-700/30">
                            <img
                              ...
                              alt="" className="h-8 w-8 rounded-full shrink-0" />
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium text-zinc-900 dark:text-white truncate">{login.discordUser.username}</p>
                              <p className="text-xs text-zinc-400">Discord linked</p>
                            </div>
                            <button onClick={unlink} className="text-xs text-red-500 hover:text-red-600 font-medium">Unlink</button>
                          </div>
                        ) : (
                          <button onClick={() => window.location.href = "/api/auth/discord/start"}
                            ...
                            <img src="/discord.svg" alt="" className="w-4 h-4 dark:invert-0 invert" />
                            Link Discord
                          </button>
                        )
                      )}
                        ...
                        login.googleUser ? (
                          <div className="flex items-center gap-3 p-3 rounded-xl border border-zinc-200 dark:border-zinc-600/60 bg-zinc-50 dark:bg-zinc-700/30">
                            <img
                              ...
                              alt="" className="h-8 w-8 rounded-full shrink-0" />
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium text-zinc-900 dark:text-white truncate">{login.googleUser.email}</p>
                              <p className="text-xs text-zinc-400">Google linked</p>
                            </div>
                            <button onClick={googleUnlink} className="text-xs text-red-500 hover:text-red-600 font-medium">Unlink</button>
                          </div>
                        ) : (
