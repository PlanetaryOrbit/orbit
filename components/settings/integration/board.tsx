...
const listboxButtonClass =
  'w-full flex items-center justify-between gap-2 px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-600 bg-white dark:bg-zinc-800 text-left text-sm font-medium text-zinc-900 dark:text-white hover:border-[color:rgb(var(--group-theme)/0.4)] focus:outline-none focus:ring-2 focus:ring-[color:rgb(var(--group-theme)/0.3)]';
...
const listboxOptionClass = (active: boolean) =>
  clsx(
    'flex items-center justify-between gap-2 px-4 py-2.5 cursor-pointer text-sm',
    active
...
const Board: FC<props> = (props) => {
  ...
  const downloadLoader = async () => {
    window.open(
      `/api/workspace/${workspace.groupId}/settings/integrations/board/download?event=${selectedSessionType}&gFormat=${globalFormat}&sClaimed=${showClaimed}&lightMode=${lightMode}`,
    );
  };
