...
import { IconGift, IconCheck, IconShield } from "@tabler/icons-react";
...
function DiscordAuditLogging({ title = "Discord Logging" }: { title?: string }) {
  ...
  useEffect(() => {
    if (router.query.id) {
      axios
        .get(
          `/api/workspace/${router.query.id}/settings/general/discord/hook`
        )
        .then((res) => {
          if (res.data.value) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Audit Logs webhook settings saved!");
    } catch (error) {
  ...
  const handleTest = async () => {
    ...
    try {
      const response = await axios.post(
        `/api/workspace/${router.query.id}/settings/general/discord/test`,
        { url: webhookUrl }
      );
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          <div className="flex items-center">
            ...
            <span className="px-2 py-1 ml-1 text-xs font-medium bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 rounded-full">
              BETA
            </span>
          </div>

          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
            Send Discord notifications when there's a new audit log
          </p>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between p-5 bg-zinc-50/80 dark:bg-zinc-900/50 rounded-xl border border-zinc-200/80 dark:border-zinc-700/50">
          <div>
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">
              Automatically send Discord messages for Audit Logs changes
            </p>
          </div>
          <button
