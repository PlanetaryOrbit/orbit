...
const TransferOwnership: FC<TransferOwnershipProps> = ({
  ...
  const handleTransfer = async () => {
    ...
    try {
      const res = await axios.patch(`/api/workspace/${workspaceId}/settings/general/transfer`, {
        ...
        <div className="flex gap-3 p-6 border-t border-gray-200 dark:border-zinc-700">
          ...
          <button
            ...
            className={clsx(
              'flex-1 px-4 py-2 text-sm font-medium rounded-md transition-colors',
              'text-white',
              selectedMemberId ? 'bg-blue-500 hover:bg-blue-600' : 'bg-gray-400 cursor-not-allowed',
              isTransferring && 'opacity-70 cursor-not-allowed',
            )}
          >
            {isTransferring ? 'Transferring...' : showConfirm ? 'Confirm Transfer' : 'Next'}
          </button>
        </div>
      </div>
    </div>
