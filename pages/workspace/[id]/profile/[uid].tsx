...
export const getServerSideProps = withPermissionCheckSsr(async ({ query, req }) => {
  ...
  const hasManagePermission =
    isAdmin ||
    (currentUser?.roles?.some((role) => role.permissions?.includes('view_member_profiles')) ??
      false);
  const hasManageMembersPermission =
    isAdmin ||
    (currentUser?.roles?.some((role) => role.permissions?.includes('edit_member_details')) ??
      false);
  const hasManageNoticesPermission =
    isAdmin ||
    (currentUser?.roles?.some((role) => role.permissions?.includes('manage_notices')) ?? false);
  const hasApproveNoticesPermission =
    isAdmin ||
    (currentUser?.roles?.some((role) => role.permissions?.includes('approve_notices')) ?? false);
  const hasRecordNoticesPermission =
    isAdmin ||
    (currentUser?.roles?.some((role) => role.permissions?.includes('record_notices')) ?? false);
  const hasActivityAdjustmentsPermission =
    isAdmin ||
    (currentUser?.roles?.some((role) => role.permissions?.includes('activity_adjustments')) ??
      false);

  const logbookPermissions = {
    view:
      isAdmin ||
      (currentUser?.roles?.some((role) => role.permissions?.includes('view_logbook')) ?? false),
    rank:
      isAdmin ||
      (currentUser?.roles?.some((role) => role.permissions?.includes('rank_users')) ?? false),
    note:
      isAdmin ||
      (currentUser?.roles?.some((role) => role.permissions?.includes('logbook_note')) ?? false),
    warning:
      isAdmin ||
      (currentUser?.roles?.some((role) => role.permissions?.includes('logbook_warning')) ?? false),
    promotion:
      isAdmin ||
      (currentUser?.roles?.some((role) => role.permissions?.includes('logbook_promotion')) ??
        false),
    demotion:
      isAdmin ||
      (currentUser?.roles?.some((role) => role.permissions?.includes('logbook_demotion')) ?? false),
    termination:
      isAdmin ||
      (currentUser?.roles?.some((role) => role.permissions?.includes('logbook_termination')) ??
        false),
    redact:
      isAdmin ||
      (currentUser?.roles?.some((role) => role.permissions?.includes('logbook_redact')) ?? false),
    delete:
      isAdmin ||
      (currentUser?.roles?.some((role) => role.permissions?.includes('logbook_delete')) ?? false),
  };
  ...
  [...roleQuotasWithInfo, ...departmentQuotasWithInfo, ...directUserQuotasWithInfo].forEach(
    (quota) => {
  ...
  try {
    ...
    const userRankRecord =
      user?.ranks?.find((r: any) => Number(r.workspaceGroupId) === workspaceGroupId) ||
      user?.ranks?.[0];
  ...
  return {
    props: {
      notices: JSON.parse(
        JSON.stringify(notices, (_k, v) => (typeof v === 'bigint' ? v.toString() : v)),
      ),
      ...
      data,
      sessions: JSON.parse(
        JSON.stringify(sessions, (_k, v) => (typeof v === 'bigint' ? v.toString() : v)),
      ),
      adjustments: JSON.parse(
        JSON.stringify(adjustments, (_k, v) => (typeof v === 'bigint' ? v.toString() : v)),
      ),
      info: {
      ...
      quotas,
      userBook: JSON.parse(
        JSON.stringify(ubook, (_k, v) => (typeof v === 'bigint' ? v.toString() : v)),
      ),
      user: {
      ...
      workspaceMember: targetUserMembership
        ? {
            ...
            lineManagerId: targetUserMembership.lineManagerId?.toString() || null,
...
const Profile: pageWithLayout<pageProps> = ({
  ...
  const currentData = {
    ...
    sessions,
    adjustments,
    messages: sessions.reduce((acc, session) => acc + Number(session.messages || 0), 0),
    idleTime: Math.round(sessions.reduce((acc, session) => acc + Number(session.idleTime || 0), 0)),
  };
      ...
      : historicalData
        ? {
            ...
            timesPlayed:
              ...
              historicalData.activity.sessionsHosted + historicalData.activity.sessionsAttended,
            data: historicalData.chartData || [0, 0, 0, 0, 0, 0, 0],
            quotas: historicalData.activity.quotaProgress
              ? Object.values(historicalData.activity.quotaProgress).map((qp: any) => ({
                  id: qp.id || qp.name || '',
                  name: qp.name || '',
                  type: qp.type || '',
