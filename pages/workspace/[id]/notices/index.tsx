...
export const getServerSideProps = withPermissionCheckSsr(async ({ params, req }) => {
  ...
  const hasApprovePermission =
    isAdmin || user?.roles.some((role) => role.permissions.includes('approve_notices'));
  const hasManagePermission =
    isAdmin || user?.roles.some((role) => role.permissions.includes('manage_notices'));
  const hasCreatePermission =
    isAdmin || user?.roles.some((role) => role.permissions.includes('create_notices'));
  ...
  const hasApproveResignations =
    isAdmin || !!user?.roles.some((r) => r.permissions.includes('approve_resignations'));
  const hasManageResignations =
    isAdmin || !!user?.roles.some((r) => r.permissions.includes('manage_resignations'));
  const hasSubmitResignation =
    isAdmin || !!user?.roles.some((r) => r.permissions.includes('submit_resignation'));
...
const Notices: pageWithLayout<NoticesPageProps> = ({
  ...
  const [userResignations, setUserResignations] = useState<ResignationWithUser[]>(
  ...
  const [activeTab, setActiveTab] = useState<'my-notices' | 'manage-notices'>('my-notices');
  const [isActiveExpanded, setIsActiveExpanded] = useState(false);
  const [isUpcomingExpanded, setIsUpcomingExpanded] = useState(false);

  const text = useMemo(() => randomText(login.displayname), []);
  const hasApproveAccess =
    canApproveNotices || workspace.yourPermission?.includes('approve_notices') || false;
  const hasManageAccess =
    canManageNoticesProp || workspace.yourPermission?.includes('manage_notices') || false;
  ...
  const showManageTab =
    hasApproveAccess ||
    hasManageAccess ||
    (resignationsEnabled && (hasApproveResignationsAccess || hasManageResignationsAccess));
  ...
  >('');
  const [resignationReviewComments, setResignationReviewComments] = useState<
    Record<string, string>
  >({});
  const [noticeReviewComments, setNoticeReviewComments] = useState<Record<string, string>>({});
  ...
  const createNotice = async () => {
    ...
    setIsCreating(true);
    try {
      const res = await axios.post(`/api/workspace/${id}/activity/notices/create`, {
  ...
  const updateNotice = async (noticeId: string, status: 'approve' | 'deny' | 'cancel') => {
    if (!id) return;
  ...
  const pendingResignations = resignationsEnabled ? allResignations.filter((r) => !r.reviewed) : [];
  const managePendingTotal = pendingNotices.length + pendingResignations.length;
