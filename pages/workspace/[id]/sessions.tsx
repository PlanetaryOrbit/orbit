...
const WeeklyCalendar: React.FC<{
  ...
  const [selectedDate, setSelectedDate] = useState<Date>(() => {
    ...
    const todayInWeek = weekDates.find((date) => date.toDateString() === today.toDateString());
  ...
  const selectedDateSessions = sessionsByDate[selectedDate.toLocaleDateString()] || [];
  useEffect(() => {
              ...
              {selectedDateSessions
                ...
                .map((session: any) => {
                  ...
                  const coHost = session.users?.find((user: any) => {
                    if (user.roleID?.toLowerCase().includes('co-host')) return true;
                    const slots = session.sessionType?.slots || [];
                    const userSlot = slots[user.slot];
                    if (userSlot?.name?.toLowerCase().includes('co-host')) return true;
                    return false;
                  });
...
const Home: pageWithLayout<pageProps> = (props) => {
  ...
  const selectedDateSessions = allSessions
    .filter((s: any) => new Date(s.date).toDateString() === selectedDate.toDateString())
  ...
  const handleDeleteSession = async (sessionId: string, deleteAll = false) => {
    try {
      await axios.delete(`/api/workspace/${router.query.id}/sessions/${sessionId}/delete`, {
        data: { deleteAll },
      });
      ...
      if (deleteAll) {
        ...
        if (session?.scheduleId) {
          ...
          setAllSessions(allSessions.filter((s) => s.scheduleId !== session.scheduleId));
        }
      } else {
