...
const Activity: pageWithLayout = () => {
  ...
  useEffect(() => {
    async function fetchUserData() {
      try {
        const profileRes = await axios.get(`/api/workspace/${id}/profile/${login.userId}`);
        const profileData = profileRes.data.data;

        const configRes = await axios.get(`/api/workspace/${id}/settings/activity/getConfig`);
              ...
              switch (quota.type) {
                ...
                case 'sessions_logged':
                  ...
                  if (quota.sessionType && quota.sessionType !== 'all') {
                    loggedCount = profileData.sessionsLogged?.byType[quota.sessionType] || 0;
                  }
                ...
                case 'alliance_visits':
                  ...
                  percentage = ((profileData.allianceVisitsCount || 0) / quota.value) * 100;
                  break;
              }
              return {
                ...quota,
  ...
  useEffect(() => {
    async function fetchLeaderboardData() {
      try {
        const configRes = await axios.get(`/api/workspace/${id}/settings/general/leaderboard`);
  ...
  const statCards = [
    {
      label: 'Active Time',
      icon: IconClock,
      value: formatMinutes(myData?.minutes ?? 0),
      desc: 'Time in-game',
    },
    ...(idleTimeEnabled
      ? [
          {
            label: 'Idle Time',
            icon: IconMoon,
            value: formatMinutes(myData?.idleTime ?? 0),
      ...
      : []),
    {
      ...
      value: (myData?.messages ?? 0).toLocaleString(),
      desc: 'Chat messages sent',
    },
    {
      label: 'Play Sessions',
      icon: IconPlayerPlay,
      value: myData?.totalPlaySessions ?? 0,
      desc: 'Total play sessions',
    },
  ];
                    ...
                    .map((user: any, i: number) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p
                            className={`mt-2 w-full truncate px-1 text-center text-xs font-bold text-zinc-900 dark:text-white sm:text-sm ${isFirst ? 'sm:text-base' : ''}`}
                          >
                            {user.username}
                          </p>
                          <p className={`text-xs font-semibold ${labelColors[pos]}`}>
              ...
              {timeline.map((item: any) => {
                if (item.__type === 'session') {
                  ...
                  const sessionDuration = isLive
                    ? Math.floor(
                        (new Date().getTime() - new Date(item.startTime).getTime()) / (1000 * 60),
                      )
                    : Math.floor(
