...
function CreateVisitModal({
  ...
  setSelectedParticipants: (v: number[]) => void;
                              ...
                              >
                                <input
                                  ...
                                  onChange={(e) =>
                                    setSelectedParticipants(
                                      e.target.checked
                                        ? [...selectedParticipants, Number(user.userid)]
...
function EditVisitModal({
  ...
  onClose: () => void;
  users: any[];
  editSelectedParticipants: number[];
  setEditSelectedParticipants: (v: number[]) => void;
  onUpdate: () => void;
                              ...
                              >
                                <input
                                  ...
                                  onChange={(e) =>
                                    setEditSelectedParticipants(
                                      e.target.checked
                                        ? [...editSelectedParticipants, Number(user.userid)]
...
function TerminationModal({
  ...
  setTermEffective: (v: string) => void;
  termReasonDraft: string;
  setTermReasonDraft: (v: string) => void;
  onConfirm: () => void;
}) {
  return (
    <Transition appear show={isOpen} as={Fragment}>
...
function StrikeModal({
  ...
  isOpen: boolean;
  onClose: () => void;
  strikeReasonDraft: string;
  setStrikeReasonDraft: (v: string) => void;
  onConfirm: () => void;
}) {
  return (
    <Transition appear show={isOpen} as={Fragment}>
...
function ManageAllyInner(props: AllyPageProps & { ally: any }) {
  ...
  const saveAllianceInfo = async () => {
    ...
    const promise = Promise.all([
      axios.post(`/api/workspace/${id}/allies/${ally.id}/update`, {
    ...
    toast.promise(promise, {
      ...
      success: 'Alliance information updated!',
  ...
  const patchStrikes = (next: number, opts?: { strikeReason?: string; onSuccess?: () => void }) => {
    ...
    if (opts?.strikeReason && clamped > strikesCount)
  ...
  const updateVisit = async () => {
    ...
    const promise = axios
      .patch(`/api/workspace/${id}/allies/${ally.id}/visits/${editContent.id}`, {
        name,
        time,
  ...
  const updateTheirRep = (i: number, value: string) =>
    setTheirReps((prev) => {
      const u = [...prev];
      u[i] = value;
      return u;
  ...
  return (
    <>
      ...
      />
      <EditVisitModal
        isOpen={editVisitOpen}
        onClose={() => setEditVisitOpen(false)}
        users={users}
        editSelectedParticipants={editSelectedParticipants}
        setEditSelectedParticipants={setEditSelectedParticipants}
      ...
      <TerminationModal
        ...
        setTermEffective={setTermEffective}
        termReasonDraft={termReasonDraft}
        setTermReasonDraft={setTermReasonDraft}
        onConfirm={submitTerminationSchedule}
      />
      <StrikeModal
        isOpen={strikeModalOpen}
        onClose={() => setStrikeModalOpen(false)}
        strikeReasonDraft={strikeReasonDraft}
        setStrikeReasonDraft={setStrikeReasonDraft}
        onConfirm={submitAddStrike}
      />
              ...
              <div className="space-y-3">
                {notes.map((note, index) => (
                  <AlliancesFormInset key={index}>
                    ...
                    {editNotes.includes(index) && (
                      <textarea
                        className={`${allianceFormInputClass} mt-2 resize-none`}
                        value={note}
