...
export const getServerSideProps = withAuthSsr(async (context: any): Promise<any> => {
  ...
  const hasRoleAccess =
    document.roles.length > 0 && document.roles.some((role: any) => userRoleIds.includes(role.id));
  const hasDepartmentAccess =
    document.departments.length > 0 &&
    document.departments.some((dept: any) => userDepartmentIds.includes(dept.id));
  ...
  return {
    props: {
      ...
      acknowledgment: existingAcknowledgment
        ? JSON.parse(
            JSON.stringify(existingAcknowledgment, (key, value) =>
              typeof value === 'bigint' ? value.toString() : value,
            ),
          )
        : null,
...
const PolicySignPage: pageWithLayout<pageProps> = ({
  document,
  alreadyAcknowledged,
  acknowledgment,
}) => {
        ...
        <div className="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-green-700 dark:text-green-300 mt-1">
                      ...
                      {new Date(acknowledgment.acknowledgedAt).toLocaleDateString()}
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-red-700 dark:text-red-300 mt-1">
                      This policy was due for acknowledgment on{' '}
                      {new Date(document.acknowledgmentDeadline!).toLocaleDateString()}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="px-6 py-4 border-t border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50">
            {alreadyAcknowledged ? (
              ...
              <div className="text-center">
                <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-4">
                  Please review the policy content above and click the button below to proceed with
                  acknowledgment.
                </p>
                <button
