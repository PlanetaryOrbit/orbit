...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const hasDiscordEnv = !!(process.env.DISCORD_APPLICATION_ID && process.env.DISCORD_SECRET);
    const hasGoogleEnv = !!(process.env.GOOGLE_APP_ID && process.env.GOOGLE_SECRET);
    ...
    if (oauthOnly && !available) {
      console.warn(
        'oauthOnly is enabled but no OAuth provider is configured. Falling back to password login',
      );
