...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  let redirectUri =
    ...
    `${process.env.PLANETARY_CLOUD_URL || process.env.NEXTAUTH_URL || process.env.PUBLIC_URL}/api/auth/roblox/callback`;
  ...
  if (!clientId || !clientSecret || !redirectUri) {
    ...
    const map = configs.reduce(
      (acc, c) => {
        acc[c.key] = typeof c.value === 'string' ? c.value.trim() : c.value;
        return acc;
      },
  ...
  try {
    ...
    const thumbnail = (await getRobloxThumbnail(Number(userId))) || undefined;
    ...
    const session = await createSession(userId, ipAddress, req.headers['user-agent']);
