...
async function syncWorkspaceMemberRankFromRobloxNoblox(
  ...
  try {
    ...
    const newRank = await rbx.getUserRank(BigInt(userId), BigInt(workspaceGroupId), ocConf.key);
...
async function parseRequestBody(req: NextApiRequest): Promise<ParsedBody> {
  ...
  const rawTargetRank = Array.isArray(fields.targetRank) ? fields.targetRank[0] : fields.targetRank;
...
async function handler(req: AuthenticatedRequest, res: NextApiResponse<Data>) {
  ...
  const promotionRankCap =
    typeof externalRanking?.rankingMaxRank === 'number' && externalRanking.rankingMaxRank >= 1
  ...
  if (
    ...
    try {
      switch (type) {
        ...
        case 'demotion':
          if (rankGunAPI && rankGun) {
            ...
            result = await rbx.demoteUser(userId, workspaceGroupId, rankingRobloxApiKey);
          } else {
            return res.status(400).json({
              success: false,
              error: 'No ranking provider configured.',
        ...
        case 'termination':
          if (rankGunAPI && rankGun) {
            ...
            result = await rbx.terminateUser(userId, workspaceGroupId, rankingRobloxApiKey);
          } else {
            return res.status(400).json({
              success: false,
              error: 'No ranking provider configured.',
      ...
      if (
        ...
        const synced = await syncWorkspaceMemberRankFromRobloxNoblox(workspaceGroupId, userId);
        if (synced) {
      ...
      let errorMessage =
        error?.response?.data?.error || error?.message || 'RankGun operation failed';
      if (typeof errorMessage === 'object') {
        try {
          errorMessage = JSON.stringify(errorMessage);
        } catch (e) {
