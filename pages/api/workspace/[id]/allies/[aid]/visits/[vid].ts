...
const withAllyPermissionCheck = (handler: any) => {
  return withAuth(async (req: AuthenticatedRequest, res: NextApiResponse) => {
    ...
    if (req.method === 'DELETE' && userrole.permissions?.includes('delete_alliance_visits'))
      return handler(req, res);
    if (req.method === 'PATCH' && userrole.permissions?.includes('edit_alliance_visits'))
      return handler(req, res);
...
export async function handler(req: AuthenticatedRequest, res: NextApiResponse<Data>) {
  ...
  if (typeof req.query.aid !== 'string')
    return res.status(400).json({ success: false, error: 'Invalid ally id' });
