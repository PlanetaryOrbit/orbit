...
export default withAuth(async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (!date || !slotId || slotIndex === undefined || !userId || timezoneOffset === undefined) {
  ...
  try {
    ...
    if (!isAdmin && userRole) {
      ...
      if (!hasPermission) {
        return res
          .status(403)
          .json({ error: "You don't have permission to assign users to this session type" });
      }
    }
    ...
    const workspaceUser = await prisma.workspaceMember.findFirst({
      where: {
        workspaceGroupId: parseInt(schedule.sessionType.workspaceGroupId.toString()),
