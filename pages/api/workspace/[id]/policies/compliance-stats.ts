...
export async function handler(req: NextApiRequest, res: NextApiResponse<ComplianceStats>) {
  if (req.method !== 'GET')
  ...
  try {
    ...
    const policyBreakdown = policies.map((policy) => {
      ...
      if (!hasRoleAssignments && !hasDepartmentAssignments) {
        ...
        const requiredMembers = members.filter((member) => {
          ...
          const hasRequiredDepartment =
            hasDepartmentAssignments && departmentUserIds.has(member.userId.toString());
      ...
      const complianceRate =
        totalRequired > 0 ? Math.min((totalAcknowledged / totalRequired) * 100, 100) : 0;

      // Calculate overdue count
      const now = new Date();
      const isOverdue =
        policy.acknowledgmentDeadline && now > new Date(policy.acknowledgmentDeadline);
      const overdueCount = isOverdue ? Math.max(totalRequired - totalAcknowledged, 0) : 0;

      // Recent acknowledgments (last 7 days)
      const sevenDaysAgo = new Date();
      sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
      const recentAcknowledgments = policyAcknowledgments.filter(
        (ack) => new Date(ack.acknowledgedAt) >= sevenDaysAgo,
      ).length;
    ...
    const memberCompliance = members.map((member) => {
      const memberAcknowledgments = allAcknowledgments.filter(
        (ack) => ack.userId.toString() === member.userId.toString(),
      );
      ...
      const overduePolicies = applicablePolicies.filter((policy) => {
        ...
        const isOverdue =
          policy.acknowledgmentDeadline && new Date() > new Date(policy.acknowledgmentDeadline);
        return !hasAcknowledged && isOverdue;
      }).length;

      const complianceRate =
        totalApplicablePolicies > 0
          ? (relevantAcknowledgedPolicies / totalApplicablePolicies) * 100
          : 100;
    ...
    const totalRequiredAcknowledgments = policyBreakdown.reduce(
      (sum, policy) => sum + policy.totalRequired,
      0,
    );
    const totalCompletedAcknowledgments = policyBreakdown.reduce(
      (sum, policy) => sum + policy.totalAcknowledged,
      0,
    );
    const overallComplianceRate =
      totalRequiredAcknowledgments > 0
        ? Math.min((totalCompletedAcknowledgments / totalRequiredAcknowledgments) * 100, 100)
        : 100;

    const pendingAcknowledgments = Math.max(
      totalRequiredAcknowledgments - totalCompletedAcknowledgments,
      0,
    );
    const overdueAcknowledgments = policyBreakdown.reduce(
      (sum, policy) => sum + policy.overdueCount,
      0,
    );
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const acknowledgementsUpToDate = allAcknowledgments.filter(
        (ack) => new Date(ack.acknowledgedAt) <= dayEnd,
      ).length;
