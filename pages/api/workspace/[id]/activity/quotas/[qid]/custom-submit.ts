...
import { getQuotaForMemberOrThrow, memberHasQuotaAssignment } from '@/utils/quotaCustomEligibility';
...
async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  if (existing?.status === 'approved') {
    return res
      .status(400)
      .json({ success: false, error: 'This quota is already approved as complete' });
  }
  ...
  try {
    ...
    try {
      await logAudit(workspaceId, uid, 'activity.quota.custom_submit', `quota:${qid}`, {
        quotaId: qid,
