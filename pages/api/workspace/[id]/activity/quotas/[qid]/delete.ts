...
export async function handler(req: AuthenticatedRequest, res: NextApiResponse<Data>) {
  ...
  try {
    const quota = await prisma.quota.findUnique({
      where: { id: req.query.qid as string },
      include: { quotaRoles: { include: { role: true } } },
    });
    ...
    try {
      await logAudit(
        ...
        'activity.quota.delete',
        `quota:${quota?.name || req.query.qid}`,
        { id: req.query.qid, name: quota?.name, type: quota?.type, value: quota?.value },
      );
    } catch (e) {}
