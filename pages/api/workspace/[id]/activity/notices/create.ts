...
export async function handler(req: AuthenticatedRequest, res: NextApiResponse<Data>) {
  ...
  if (!req.body.startTime || !req.body.endTime || !req.body.reason)
    return res.status(400).json({ success: false, error: 'Missing data' });
  if (typeof req.body.startTime !== 'number' || typeof req.body.endTime !== 'number')
    return res.status(400).json({ success: false, error: 'Invalid type(s)' });
