...
async function handler(req: AuthenticatedRequest, res: NextApiResponse) {
  ...
  const { id } = req.query;
  const { userId, minutes, reason, action } = req.body as {
    userId?: number;
    minutes?: number;
    reason?: string;
    action?: 'award' | 'remove';
  ...
  if (minutes > 1000)
    return res
      .status(400)
      .json({ success: false, error: 'Minutes cannot exceed 1000 per adjustment' });
  if (action && !['award', 'remove'].includes(action))
  ...
  try {
    await logAudit(
      ...
      'activity.adjustment',
      `user:${userId}`,
      { minutes: signedMinutes, reason: reason || null, actorId, userId },
    );
  } catch (e) {}

  const safe = JSON.parse(
    JSON.stringify(adjustment, (_k, v) => (typeof v === 'bigint' ? v.toString() : v)),
  );
