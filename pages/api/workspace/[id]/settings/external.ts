...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    const tokenTrim = typeof body.rankingToken === 'string' ? body.rankingToken.trim() : '';
    const wsTrim =
    ...
    if (rankingProvider === 'rankgun') {
      ...
      if (!effectiveToken || !effectiveWs) {
        return res.status(400).json({ message: 'RankGun requires both API key and workspace ID' });
      }
    }
