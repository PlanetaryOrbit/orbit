...
export default async function handler(
  ...
  if (req.method === 'GET') {
    const config = await getConfig('policies', parseInt(req.query.id as string));
    if (!config) {
  ...
  return withPermissionCheck(async (req: NextApiRequest, res: NextApiResponse<Data>) => {
    if (req.method === 'PATCH') {
      ...
      try { await logAudit(workspaceId, (req as any).session?.userid || null, 'settings.general.policies.update', 'policies', { before, after }); } catch (e) {}
