...
export async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  try {
    ...
    if (!workspaceId)
      return res.status(400).json({ success: false, error: 'No workspace id provided' });
    if (!newOwnerId)
      return res.status(400).json({ success: false, error: 'No new owner id provided' });
    ...
    if (!currentOwnerMembership?.isAdmin) {
      return res
        .status(403)
        .json({ success: false, error: 'Only workspace owners can transfer ownership' });
    }
    ...
    if (!newOwnerMembership) {
      return res
        .status(404)
        .json({ success: false, error: 'New owner is not a member of this workspace' });
    }
    if (newOwnerMembership.userId === userId) {
      return res
        .status(400)
        .json({ success: false, error: 'New owner must be different from current owner' });
    }
