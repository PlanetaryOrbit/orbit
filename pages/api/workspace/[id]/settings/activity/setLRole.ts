...
export async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  if (role !== null && role !== undefined && typeof role !== 'number') {
    return res.status(400).json({ success: false, error: 'Leaderboard role must be a number' });
  }

  const currentConfig = (await getConfig('activity', parseInt(req.query.id as string))) || {};
  const updatedConfig = {
    ...currentConfig,
    lRole: role,
  };
  ...
  try {
    await logAudit(
      ...
      'settings.activity.lRole.update',
      'activity.lRole',
      { before: currentConfig, after: updatedConfig },
    );
  } catch (e) {}
