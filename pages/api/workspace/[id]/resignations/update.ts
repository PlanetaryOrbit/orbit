...
export async function handler(req: AuthenticatedRequest, res: NextApiResponse<Data>) {
  ...
  if (status === 'cancel') {
    ...
    const hasManagePermission =
      isAdmin || user?.roles.some((role) => role.permissions.includes('manage_resignations'));

    if (!hasManagePermission) {
      return res.status(403).json({
        success: false,
        error: 'Insufficient permissions. Removing a resignation requires manage_resignations.',
      });
    }
  }
