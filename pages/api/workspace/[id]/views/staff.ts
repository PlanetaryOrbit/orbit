...
export default withPermissionCheck(async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    const computedFilters = filters.filter((f) => !['username'].includes(f.column));
    ...
    for (const user of allUsers) {
      ...
      allActivity
        ...
        .forEach((session) => {
          ...
          const idleTimeMs =
            idleTimeEnabled && session.idleTime ? Number(session.idleTime) * 60000 : 0;
      ...
      const userDepartments =
        user.workspaceMemberships?.[0]?.departmentMembers?.map((dm: any) => dm.department.id) || [];
    ...
    if (needsFullComputation) {
      ...
      paginatedUsers = filteredUsers.slice(page * pageSize, (page + 1) * pageSize);
    } else {
