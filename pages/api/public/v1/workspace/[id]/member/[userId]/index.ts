...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method == 'GET') {
    ...
    const { lineManager, discordId, timezone, birthdayDay, birthdayMonth } = req.body;
    if (lineManager && typeof lineManager !== 'string' && typeof lineManager !== 'number') {
      return res.status(400).json({ success: false, error: 'Invalid lineManager ID' });
    }
