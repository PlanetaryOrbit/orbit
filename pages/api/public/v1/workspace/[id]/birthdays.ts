...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const daysParam = req.query.days ? Number.parseInt(req.query.days as string) : 7;
    const days = isNaN(daysParam) ? 7 : Math.min(Math.max(daysParam, 1), 14);
    const specificDate = req.query.date ? new Date(req.query.date as string) : null;
    ...
    const filtered = memberBirthdays
      ...
      .map((m) => {
        ...
        const todayNormalized = new Date(todayYear, today.getMonth(), today.getDate());
