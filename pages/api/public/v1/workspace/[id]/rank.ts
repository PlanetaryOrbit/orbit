...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!userid && userid.length > 0)
    return res.status(400).json({ success: false, error: 'Missing user ID in request body.' });
  if (!roleid && roleid.length > 0)
    return res.status(400).json({ success: false, error: 'Missing roleid in request body.' });

  try {
    const config = await getConfig('roblox_opencloud', workspaceId);
    if (!config)
      return res.status(400).json({ success: false, error: 'Opencloud keys could not be found.' });
    if (!config.enabled)
      return res.status(400).json({ success: false, error: 'Opencloud keys are disabled.' });
    ...
    try {
      ...
      if (!targetRole)
        return res.status(400).json({ success: false, error: 'Role does not exist on the group.' });

      const admins = await Client.groups.listGroupMemberships(workspaceId.toString(), {
        filter: `role == 'groups/${workspaceId}/roles/${roleid}'`,
      });

      if (admins.groupMemberships.find((user) => user.user == `users/${userid}`))
        return res.status(400).json({ success: false, error: 'User is already ranked.' });
